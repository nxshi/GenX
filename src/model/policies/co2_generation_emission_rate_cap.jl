...
function co2_generation_side_emission_rate_cap!(EP::Model, inputs::Dict, setup::Dict)
    ...
    POWER = union(THERM_ALL, VRE, MUST_RUN, HYDRO_RES)
    
    ##CO2 Polcy Module Generation by zone
    if !isempty(POWER)
        @expression(EP, eGenerationByZone[z = 1:Z, t = 1:T], # the unit is GW
            sum(EP[:vP][y, t] for y in intersect(POWER, dfGen[dfGen[!, :Zone].==z, :R_ID]))
        )
    end
    ### Constraints ###

    ## Generation + Rate-based: Emissions constraint in terms of rate (tons/MWh)
    @constraint(EP, cCO2Cap_genrate[cap=1:inputs["NCO2GenRateCap"]],
