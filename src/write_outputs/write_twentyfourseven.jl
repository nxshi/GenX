...
function write_twentyfourseven(path::AbstractString, inputs::Dict, setup::Dict, EP::Model)
    ...
    if (NumberofTFS) > 1
        ...
        if setup["ParameterScale"] == 1
            temptfsflow *= ModelScalingFactor
        end
        ...
        if setup["ParameterScale"] == 1
            temptfsexport *= ModelScalingFactor
        end
        ...
        if setup["ParameterScale"] == 1
            temptfstranscationcost *= ModelScalingFactor^2
        end
    ...
    for rpsh in 1:NumberofTFS
        ...
        if setup["ParameterScale"] == 1
            temprevenue *= ModelScalingFactor^2
        end
